...
async def broadcast(tipo: str, data: dict = None):
    if data is None:
        data = {}
...
# ================= FUNÇÕES PLAYWRIGHT =================
async def sessao_valida(playwright):
...
async def worker(id_worker, browser, fila):
    ...
    try:
        ...
        context = await browser.new_context(storage_state=str(SESSAO_PATH), viewport=VIEWPORT, user_agent=USER_AGENT)
        page = await context.new_page()
        page.on("dialog", lambda d: asyncio.create_task(d.accept()))
...
async def processar_todos_cartoes(texto_cartoes, num_canais):
    try:
